...
export default function LoginPage() {
  ...
  return (
    <div className="flex flex-col h-screen items-center justify-center">
      ...
      <Form method="post" className=" small-form border-[0.05rem] border-neutral-950 w-1/3 p-8 rounded flex flex-col justify-left items-left" >
        {actionData?.message && (<p className="text-red-500">{actionData.message}</p> )}

        <p className="m-2">
          <label htmlFor="username" className="mr-4 font-semibold"> Username </label>
          <input
            type="text"
            id="userName"
        ...
        <p className="m-2">
          <label htmlFor="password" className="mr-4 font-semibold"> Password </label>
          <input
            type="text"
            id="password"
            className="bg-white rounded p-2 w-[100%] text-black font-semibold"
...
/** Request: This is the Fetch API Request object, representing an HTTP request. When you write { request: Request } in a function, it specifies,
    ...
    export async function action({ request } : { request: Request }) {
      ...
      //--------< checking username and  password >------------
        ...
        if (inputUserName === "ipgautomotive" && inputPassword === "carmaker") {
          ...
          return { message: "We couldn't log you in. Double-check your username and password, and Try again." };
        } 
